...
export class Query {
  ...
  addFilter(filter: Filter): Query {
    ...
    softAssert(!this.isDocumentQuery(), 'No filtering allowed for document query');
  ...
  addOrderBy(orderBy: OrderBy): Query {
    softAssert(!this.startAt && !this.endAt, 'Bounds must be set after orderBy');
...
export class KeyFieldFilter extends FieldFilter {
  ...
  constructor(field: FieldPath, op: Operator, value: api.Value) {
    ...
    softAssert(isReferenceValue(value), 'KeyFieldFilter expects a ReferenceValue');
